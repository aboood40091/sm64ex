...
static void alloc_languages(char *gameDir) {
    if (!languages) {
    ...
    snprintf(languagesDir, FILENAME_MAX, "%s/%s/texts/", sys_exe_path(), gameDir);
...
void alloc_dialog_pool(char *gamedir) {
    alloc_languages(gamedir);

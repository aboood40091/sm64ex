...
RENDER_API ?= GL
# Window managers: SDL2, DXGI (forced if D3D11 or D3D12 in RENDER_API), WHB (forced if the target is Wii U)
WINDOW_API ?= SDL2
# Audio backends: SDL2
AUDIO_API ?= SDL2
...
VERSION_CFLAGS := $(VERSION_CFLAGS) -D_LANGUAGE_C -DNON_MATCHING -DAVOID_UB
...
DUMMY != $(MAKE) -s -C $(TOOLS_DIR) -j1 >&2 || echo FAIL
ifeq ($(DUMMY),FAIL)
...
.PHONY: all clean cleantools distclean default diff test load libultra res
